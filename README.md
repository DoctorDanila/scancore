...
***Изменение назначения раздела***: до этого раздел декларировал изменения конкретно документа `README.md`.
С настоящего момента, ***данный раздел декларирует изменения в текущей версии*** инструмента

- Добавлена поддержка временных правил игнорирования из командной строки через флаг `--ignore=шаблон`. Можно использовать многократно.
- Введён файл `.env` для настройки поведения: переменная `IGNORE_EXCEPT_COMMANDS` позволяет указать команды, для которых временные игноры не применяются (по умолчанию `init`).
- Если `.env` отсутствует, при использовании `--ignore` выводится предупреждение, и временные правила игнорируются (сканирование идёт только по `.scancoreignore`).
- Обновлены все команды для работы с новым парсером аргументов `Input` и обработчиком временных исключений.
- Добавлены классы: `Input`, `EnvLoader`, `IgnoreOptionHandler`.
- Расширен `IgnoreRules` методом `addPatterns()` для добавления временных паттернов.
- В конструктор `Scanner` добавлен необязательный параметр `$additionalPatterns`.

...
**Новые возможности анализа и визуализации:**
- **Исправлен анализ зависимостей**: корректно обрабатываются конструкции `extends`, `implements`.
- **Полный учёт всех типов связей**:
  - `require`/`include` - теперь распознаются даже динамические пути с `__DIR__`.
  - `import` (операторы `use`) - добавлен отдельный граф импортов.
  - `usage` - вызовы через `new`, `::`, `instanceof`, `catch`.
- **Доработана обработка статических вызовов** (`::`): анализатор теперь корректно определяет класс слева от оператора, что позволяет видеть зависимости от классов, используемых в статических вызовах (например, `EnvLoader::load()`).

- **Обновлена страница статистики**:
  - HTML-отчёт полностью переработан, представление вынесено в отдельный файл `resources/views/stats.php`.
  - Библиотека визуализации заменена на **Cytoscape.js** (ранее использовалась vis-network).
  - Добавлена возможность включать/выключать отображение рёбер по типам связей (фильтрация).
  - Реализована кластеризация по директориям: узлы группируются в папки, которые можно сворачивать и разворачивать для улучшения читаемости больших графов.
  - Добавлен футер с указанием компании «Script Agency» и ссылкой на сайт.

- **Исправлена ошибка обработки include/require**: теперь динамические пути, формируемые с помощью `__DIR__`, корректно вычисляются и добавляются в граф зависимостей.

- **Улучшена обработка оператора `::` (двойное двоеточие)**: анализатор правильно распознаёт имя класса слева, что позволяет учитывать зависимости от классов при статических вызовах.
